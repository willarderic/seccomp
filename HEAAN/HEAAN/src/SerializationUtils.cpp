...
void SerializationUtils::writeCiphertext(Ciphertext& cipher, std::string path) {
	std::fstream fout;
	...
	NTL::ZZ q = NTL::conv<NTL::ZZ>(1) << logq;
...
Ciphertext* SerializationUtils::readCiphertext(std::string path) {
	long n, logp, logq;
	std::fstream fin;
...
void SerializationUtils::writeKey(Key* key, std::string path) {
	std::fstream fout;
	fout.open(path, std::ios::binary|std::ios::out);
...
Key* SerializationUtils::readKey(std::string path) {
	Key* key = new Key;
	std::fstream fin;
	fin.open(path, std::ios::binary|std::ios::in);
